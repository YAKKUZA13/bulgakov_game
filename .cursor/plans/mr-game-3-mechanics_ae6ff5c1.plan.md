---
name: mr-game-3-mechanics
overview: "Перестроить текущий web‑демо (Three.js + Depth Anything) в MR‑игровую базу с непрерывным трекингом позы в браузере (без ARCore/ARKit), реконструкцией/детекцией поверхностей реального мира, интеграцией физики и тремя механиками: runner‑кубик по поверхностям, «angry birds» на примитивах и поиск предметов по позе телефона."
todos:
  - id: tracking_webslam
